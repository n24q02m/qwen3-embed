...
from qwen3_embed.common.onnx_model import OnnxModel, OnnxOutputContext, OnnxSessionConfig
...
def test_load_cuda_and_providers_warning(model: ConcreteOnnxModel, mock_ort):
    ...
    with pytest.warns(UserWarning, match="`cuda` and `providers` are mutually exclusive"):
        model._load_onnx_model(
            Path("dummy"),
            "model.onnx",
            OnnxSessionConfig(threads=None, cuda=True, providers=["CUDAExecutionProvider"]),
        )
...
def test_load_extra_session_options(model: ConcreteOnnxModel, mock_ort):
    ...
    model._load_onnx_model(
        Path("dummy"),
        "model.onnx",
        OnnxSessionConfig(threads=None, extra_session_options={"enable_cpu_mem_arena": False}),
    )
