...
def test_add_extra_session_options():
    ...
    # Test valid options
    ConcreteOnnxModel.add_extra_session_options(
        session_options,
    ...
    # Test invalid option
    with pytest.raises(ValueError, match="invalid_option is unknown or not exposed \\(exposed options: \\('enable_cpu_mem_arena',\\)\\)"):
        ConcreteOnnxModel.add_extra_session_options(
            session_options,
            {"invalid_option": True}
        )
