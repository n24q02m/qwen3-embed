...
class TestOnnxTextModelEmbedDocuments:
    ...
    def test_single_string_becomes_one_result(self) -> None:
        m = self._loaded(1)
        results = list(m._embed_documents(TextModelConfig("t", "/tmp"), documents="hello"))
    ...
    def test_small_list_all_results(self) -> None:
        ...
        m = self._loaded(2)
        results = list(m._embed_documents(TextModelConfig("t", "/tmp"), documents=["a", "b"]))
        assert len(results) == 1
    ...
    def test_loads_model_if_none(self) -> None:
        ...
        list(m._embed_documents(TextModelConfig("t", "/tmp"), documents=["hi"]))
        assert loaded
