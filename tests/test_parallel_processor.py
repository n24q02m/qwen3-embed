...
def test_worker_process_exception_handling():
    """Test that _worker handles exceptions during Worker.process() gracefully."""
    input_queue = MagicMock()
    # Mock _get_items_from_queue to return an iterator that yields one item then stops
    input_queue.get.side_effect = [(0, 1), QueueSignals.stop]
