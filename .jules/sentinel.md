...
## 2026-08-03 - [Fix arbitrary file write vulnerability by prohibiting symlinks in manual tar extraction]
**Vulnerability:** The manual fallback path for tar extraction permitted symlinks and hardlinks in `_validate_tar_member`, posing a risk for arbitrary file writes if a malicious archive bypassed manual path validation logic.
**Learning:** Permitting symlinks in generic, untrusted archive extraction on systems without native `data` filters invites traversal risks. Secure implementations for model caching generally do not need to support symlinks.
**Prevention:** Strictly prohibit symlinks (`.issym()`) and hardlinks (`.islnk()`) during untrusted tarball extraction. Only allow regular files and directories.
