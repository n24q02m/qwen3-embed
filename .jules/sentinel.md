...
## 2026-08-05 - [Local DoS during cache cleanup]
**Vulnerability:** `shutil.rmtree` could crash with a `NotADirectoryError` if `model_tmp_dir` was pre-created as a file or symlink.
**Learning:** Checking `.exists()` is not sufficient before calling `shutil.rmtree()`, as it assumes the path is a directory.
**Prevention:** First check if the path is a symlink or a non-directory file, and use `unlink()` to remove it safely before falling back to `shutil.rmtree()`.
