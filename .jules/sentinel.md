...
## 2026-07-06 - [Path Traversal bypass via Backslash Injection]
**Vulnerability:** When sanitizing paths or filenames for cache generation using `model_name.split('/')[-1]`, attackers can bypass the check on Windows systems (or systems where `Path` resolves `\` as a separator) by injecting backslashes (e.g., `foo\..\..\etc\passwd`).
**Learning:** `split('/')` alone is insufficient if the target filesystem or path parsing libraries (like `pathlib.Path`) treat backslashes as valid directory separators.
**Prevention:** Always normalize separators (e.g., `replace('\\', '/')`) before splitting, or explicitly validate against allowed character sets when generating filenames from untrusted input.
