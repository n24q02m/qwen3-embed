...
## 2026-07-07 - [Symlink bypass in file removal]
**Vulnerability:** The archive cleanup logic in `_prepare_gcs_cache` used `exists()` before unlinking `model_tar_gz`. `exists()` returns `False` for dangling symlinks, meaning a malicious dangling symlink left at the download path would not be removed, potentially causing denial of service or arbitrary file overwrites upon download completion.
**Learning:** `Path.exists()` evaluates to `False` if the path points to a dangling symlink, so it cannot be used alone to check if a path is occupied.
**Prevention:** Use `Path.is_symlink() or Path.exists()` or `os.path.lexists()` when checking if a path needs to be cleared before creating a file or directory.
