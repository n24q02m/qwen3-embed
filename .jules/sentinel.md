...
## 2026-07-06 - [Fix Symlink DoS in shutil.rmtree]
**Vulnerability:** In `_prepare_gcs_cache`, `shutil.rmtree(model_tmp_dir)` was called guarded only by `.exists()`. If an attacker or a previous crashed process left a symlink or a regular file at that path, `rmtree` would crash with `NotADirectoryError` or similar, leading to a local Denial of Service (DoS) during model caching. Furthermore, `exists()` returns false for broken symlinks, leaving them undeleted.
**Learning:** `shutil.rmtree` is strictly for directories. Naive `.exists()` checks do not account for the file type or broken symlinks.
**Prevention:** Always verify if a path `.is_symlink()` or is not a directory before invoking `rmtree`, and use `.unlink()` to safely remove symlinks or files.
