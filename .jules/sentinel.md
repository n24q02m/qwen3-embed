...
## 2026-07-28 - [DoS via Cache Cleanup Symlinks]
**Vulnerability:** In `_prepare_gcs_cache`, `shutil.rmtree` was used on a directory path without verifying if it was a directory or a symlink. If an attacker places a symlink or a file at that path, it causes an unexpected `NotADirectoryError` or `OSError` crashing the application (Local DoS).
**Learning:** `shutil.rmtree` assumes the target is a directory and fails if it is a symlink or file.
**Prevention:** Check if the path is a symlink or a non-directory file using `path.is_symlink()` and `not path.is_dir()`, and use `path.unlink()` to remove it safely before falling back to `shutil.rmtree()`.
