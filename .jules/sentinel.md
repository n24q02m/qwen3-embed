...
## 2024-10-24 - [Local DoS in Cache Cleanup]
**Vulnerability:** `shutil.rmtree()` in cache cleanup is vulnerable to `NotADirectoryError` if a symlink or file exists at the expected directory path.
**Learning:** Always check `path.is_symlink()` or `(path.exists() and not path.is_dir())` and call `path.unlink()` to prevent local Denial of Service (DoS) when cleaning up cache directories.
**Prevention:** Use `path.unlink()` for symlinks/files and `shutil.rmtree` only for directories when cleaning up caches.
