...
## 2026-07-27 - [Fix DoS via cache cleanup symlink/file conflict]
**Vulnerability:** `shutil.rmtree` was called on `model_tmp_dir` without checking if it was a directory.
**Learning:** Calling `rmtree` on a symlink or file throws `OSError` or `NotADirectoryError`, causing a DoS.
**Prevention:** Check `is_symlink()` and `is_dir()` and safely `unlink()` files/symlinks before attempting `rmtree`.
