...
## 2024-10-24 - [DoS via Symlink Overwrite during Cache Cleanup]
**Vulnerability:** When cleaning up the model temporary cache using `shutil.rmtree(model_tmp_dir)`, if an attacker or process replaces `model_tmp_dir` with a symlink or regular file, `shutil.rmtree` raises a `NotADirectoryError` (Python 3.12+) or `OSError`, crashing the process and causing a Denial of Service.
**Learning:** `shutil.rmtree` assumes the target is a directory and fails if it is not.
**Prevention:** Always check if the target path is a symlink (`is_symlink()`) or a non-directory file (`exists() and not is_dir()`) and call `unlink()` before falling back to `shutil.rmtree()`.
