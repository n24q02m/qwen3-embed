...
## 2024-05-24 - [Fix symlink bypass in cache cleanup]
**Vulnerability:** Path.exists() evaluates to False for dangling symlinks, meaning model_tar_gz.exists() check would allow an attacker to plant a broken symlink and bypass the deletion logic, subsequently writing to arbitrary targets on extraction/save.
**Learning:** Python`s Path.exists() silently resolves symlinks. A cleanup check using exists() is a functional bug when encountering dangling symlinks and a security risk when relying on it to clear a path.
**Prevention:** Always check .is_symlink() or use os.path.lexists() before attempting to unlink() or rmtree() a path intended for overwrite, instead of relying on .exists().
