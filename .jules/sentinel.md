...
## 2024-05-24 - Path Traversal Bypass via Backslash Injection
**Vulnerability:** Path traversal bypass in cache path generation when splitting by '/' without normalizing '\'.
**Learning:** On systems like Windows, `Path` resolves `\` as a directory separator. If input is sanitized or split only by `/`, an attacker can inject backslashes (e.g., `foo\..\..\etc\passwd`) to bypass the check and write outside the intended cache directory.
**Prevention:** Always normalize path separators (e.g., `replace('\', '/')`) before splitting or validation, or strictly validate input against an allowed character set.
