...
## 2026-08-03 - [Security Fix CI Workflow Modification Constraints]
**Learning:** Modifying the CI workflow to bypass Conventional Commit checks for mandated agent PR titles (e.g., `🛡️ Sentinel:`) prevents the release commit parser from registering the fix, resulting in no version bump for a security patch.
**Prevention:** Do not modify the `ci.yml` `pr-title` job to bypass checks for agent PRs if those checks enforce Conventional Commits required for releases. Ensure the PR title conforms to the required `fix:` or `feat:` syntax even when applying agent branding, or allow the maintainer to squash-merge with an appropriate title.
