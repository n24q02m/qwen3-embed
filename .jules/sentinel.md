...
## 2024-10-24 - [Fix symlink bypass in cache cleanup]
**Vulnerability:** `Path.exists()` returns `False` for dangling symlinks, allowing an attacker to plant a dangling symlink at `model_tar_gz` that escapes cleanup in `_prepare_gcs_cache`.
**Learning:** Checking `.exists()` alone is insufficient when dealing with potential symlink attacks, as a dangling symlink will falsely report as non-existent and bypass cleanup logic, setting up subsequent vulnerabilities (like arbitrary file writes during extraction or downloads).
**Prevention:** Always use `.is_symlink() or .exists()` (or `os.path.lexists()`) when verifying file presence for secure cleanup or before atomic operations.
