...
## 2026-07-04 - [Fix SSRF prevention logic to support valid ports]\n**Vulnerability:** SSRF prevention logic used `.netloc` which was brittle and subject to functional false negatives or bypasses via userinfo/port syntax.\n**Learning:** Relying on strict string matching against `.netloc` (e.g. `!= "storage.googleapis.com"`) rejects valid URLs specifying ports and sets up fragile patterns that are easily broken. `.hostname` extracts the clean domain.\n**Prevention:** Use `urllib.parse.urlparse(url).hostname` instead of `.netloc` for robust hostname validation against a known allowed list.
## 2024-10-24 - [Cache Cleanup NotADirectoryError / OSError DoS]
**Vulnerability:** When cleaning up the GCS cache directory, `shutil.rmtree` was called unconditionally on `model_tmp_dir` if it existed. This could lead to a Denial of Service (DoS) if an attacker or another process pre-created the path as a symlink or a regular file, causing `shutil.rmtree` to crash with `NotADirectoryError` or `OSError`.
**Learning:** `shutil.rmtree` expects a directory. If the path exists but is not a directory (e.g. a symlink or file), it crashes. Additionally, `path.exists()` returns `False` for broken symlinks, potentially bypassing cleanup completely and causing later operations to fail.
**Prevention:** When cleaning up dynamic temporary directories, explicitly check `is_symlink()` and `is_dir()`. Use `unlink()` to remove symlinks or regular files before falling back to `rmtree()` for directories.
