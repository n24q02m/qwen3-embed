...
## 2024-10-24 - [Path Traversal in Cache Generation]
**Vulnerability:** Sanitizing paths using `split('/')` allows attackers to bypass directory traversal checks on systems that support backslashes as path separators (like Windows) by injecting `\` (e.g., `foo\..\..\etc\passwd`).
**Learning:** `Path` resolves `\` as a separator on Windows, so manual sanitization must handle all possible separators.
