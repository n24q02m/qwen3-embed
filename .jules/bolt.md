...
## 2024-05-24 - [Fast C-level substring check before sanitization]
**Learning:** When sanitizing text against multiple specific tokens with a common prefix (e.g., Qwen forbidden tokens like `<|im_start|>`), adding a broad C-level substring fast-path (e.g., `if "<|" not in text: return text`) before an `any(...)` or `re.search(...)` loop significantly reduces Python iteration overhead on typical clean inputs.
**Action:** Always include a defensive runtime assertion at the module level (e.g., `assert all('<|' in t for t in FORBIDDEN_TOKENS)`) to prevent silent security bypasses if the token list is modified later, and implement the fast-path string matching to bypass slower checks.
## 2026-07-23 - [Do not micro-optimize string sanitization on cold paths]
**Learning:** Adding a C-level substring fast-path (e.g., `if "<|" not in text: return text`) before an `any(...)` or `re.search(...)` loop in `_sanitize_input` provides a measurable per-call speedup (~15-20%). However, this function is called only three times per forward pass, which takes ~8.3 seconds. Saving a few microseconds here yields a 0.00003% overall improvement and introduces a severe security risk if new forbidden tokens (without the `<|` prefix) are added in the future, as `assert` statements used to guard this assumption are stripped when Python is run with the `-O` flag.
**Action:** Avoid micro-optimizing string replacement or sanitization functions on paths dominated by heavy ML inference, especially when such optimizations rely on fragile assumptions about security token formats.
