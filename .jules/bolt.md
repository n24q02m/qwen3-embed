...
## 2024-05-27 - [Fast single-batch scalar math using math.exp]
**Learning:** For single scalar mathematical operations (e.g., `batch_size == 1` logit differences in `_compute_yes_no_scores`), using Python's built-in `math.exp(float(val))` wrapped in a `try...except OverflowError` block is ~4-5x faster than `numpy.exp(array)`. This is because it completely bypasses NumPy's C-API dispatch, type checking, and array object creation overhead.
**Action:** When performing mathematical operations on scalar arrays or batches of size 1, conditionally branch to use the standard library `math` module instead of `numpy` functions for significant speedups.
