...
## 2024-05-13 - [Fast Right-Padding Index Calculation]
**Learning:** When calculating the last token index for strictly right-padded attention masks (contiguous 1s followed by 0s), `attention_mask.sum(axis=1) - 1` avoids the array reversal (`[::-1]`) and argmax allocation overhead, proving significantly faster (~5x) in hot inference loops.
**Action:** Use `attention_mask.sum(axis=1) - 1` for right-padded sequences instead of `np.argmax(attention_mask[:, ::-1], axis=1)`.
