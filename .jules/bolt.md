...
## 2026-06-25 - [Fast token logit extraction without full vocabulary slice allocation]
**Learning:** When extracting specific token logits from the last position of a causal LM output, avoiding full vocabulary slice allocation (e.g. `last_logits = model_output[:, -1, :]` which has shape `(batch_size, vocab_size)`) is much faster. By directly slicing the specific dimensions for `TOKEN_YES_ID` and `TOKEN_NO_ID` (e.g. `model_output[:, -1, TOKEN_NO_ID]`), we prevent allocating memory for the unused vocabulary, yielding over a 10x speedup when not left-padded and >10,000x for very large vocabs or outputs.
**Action:** Extract only the necessary element indices from multidimensional numpy arrays when subsequent steps only require those specific elements, avoiding memory allocation for large unused dimensions.
