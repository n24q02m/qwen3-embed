...
## 2026-06-30 - [Fast mask summation before casting]
**Learning:** When dealing with boolean/integer masks (like `attention_mask`), calculating the sum of the mask before casting it to a wider floating-point type (e.g., `np.float32`) is about ~4x faster than casting the entire 2D mask array to a float type and then summing it. This avoids allocating a large intermediate float array just for the reduction.
**Action:** When a sum reduction is needed on an integer/boolean array that ultimately needs to be a float, perform `.sum()` on the original array first, and then `.astype()` on the significantly smaller reduced result.
