...
## 2024-05-24 - [Fast last token index calculation in right-padded masks]
**Learning:** When calculating the last token index for strictly right-padded attention masks (which consist of contiguous 1s followed by 0s) in numpy, use `mask.sum(axis=1) - 1` instead of `seq_len - 1 - np.argmax(mask[:, ::-1], axis=1)`. This avoids array reversal and argmax allocation overhead, making it significantly (~5x) faster in hot inference loops.
**Action:** Use `mask.sum(axis=1) - 1` to find the last valid token index for strictly right-padded masks.
