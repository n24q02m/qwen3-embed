...
## 2024-05-30 - Fast scalar math operations
**Learning:** Python's built-in `math.exp` is significantly faster (~6x) than `np.exp` when dealing with single scalar values because it avoids the overhead of Numpy's ufunc dispatching and array object creation.
**Action:** When performing math operations on extracted scalar floats in inner loops, use Python's built-in `math` module instead of `numpy` functions. Make sure to handle `OverflowError` appropriately.
