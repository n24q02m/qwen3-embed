...
## 2026-06-06 - [Fast string replacement for prompt injection sanitization]
**Learning:** When sanitizing strings by iteratively removing multiple literal tokens, using a nested `while True` loop with `str.replace()` on each token is significantly faster than using a compiled regular expression's `subn()` method. Regular expressions incur higher overhead for simple literal replacements.
**Action:** Use `str.replace()` for iteratively removing a fixed set of literal substrings instead of regular expressions when performance is critical.
