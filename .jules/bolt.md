...
## 2026-07-27 - [Avoid manual string fast-paths over self-maintaining regex]
**Learning:** Replacing a self-maintaining, list-driven regex `search` fast-path with a manually hardcoded string literal check (e.g. `if "<|" not in text:`) introduces a silent prompt-injection vulnerability risk. Because `assert` statements used to enforce such invariants are stripped out under the `-O` optimization flag, adding a new token to the list that doesn't contain the hardcoded prefix would bypass sanitization entirely in production without any errors. Furthermore, saving ~2.5 microseconds on a fast-path that precedes a massive 8.3-second ML inference call provides 0% measurable throughput improvement.
**Action:** Never trade a self-maintaining security guard for a manual one just for micro-optimizations, especially when the optimization is located on a cold path relative to massive network or ML inference calls. Always consider if the performance win actually impacts end-to-end throughput before degrading maintainability or safety.
