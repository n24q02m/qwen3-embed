...
## 2024-05-11 - Fast last token index extraction
**Learning:** For strictly right-padded attention masks (which consist of contiguous 1s followed by 0s), computing the index of the last token using `attention_mask.sum(axis=1) - 1` is significantly faster than `seq_len - 1 - np.argmax(attention_mask[:, ::-1], axis=1)`. This avoids array reversal and argmax allocation overhead.
**Action:** Replace `seq_len - 1 - np.argmax(attention_mask[:, ::-1], axis=1)` with `attention_mask.sum(axis=1) - 1` for right-padded sequences in numpy implementations.
