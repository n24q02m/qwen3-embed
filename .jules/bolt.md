...
## 2026-06-08 - Optimized Model Registry Aggregation
**Learning:** `list.extend()` in a loop is less efficient than `itertools.chain.from_iterable()` for flattening nested collections in Python.
**Action:** Refactored `_list_supported_models` in `TextEmbedding` and `TextCrossEncoder` to use `itertools.chain.from_iterable` for better performance and readability.
