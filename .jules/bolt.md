...
## 2026-07-25 - [Reverted fast-path substring check due to security and micro-optimization concerns]
**Learning:** Adding a C-level substring check (`"<|" not in text`) before regex sanitization for prompt injection protection was reverted. While it offered a microsecond-level performance win in isolation, it introduced a severe security risk if the invariant (all forbidden tokens containing `"<|"`) was ever violated. The module-level `assert` added to guard this invariant is removed under `-O` (optimized execution), meaning a prompt injection bypass could silently occur. Additionally, the micro-optimization saved only ~2.5 microseconds per call, which is negligible (0.00003%) compared to the 8.3-second ML model forward pass, meaning throughput was fundamentally unchanged.
**Action:** Do not implement micro-optimizations that couple security functions to brittle invariants, especially when the total time saved is negligible compared to the application's overall bottleneck. Assertions should not be used as the sole runtime security boundary, as they are stripped in optimized execution environments.
