...
## 2026-06-10 - Simplicity before Speculative Abstraction
**Learning:** Avoid introducing complex abstractions (like lazy sequences) to solve hypothetical performance issues without empirical data (profiling/benchmarks). If the intermediate data size is bounded (e.g., by batch_size) and the downstream operation (e.g., tokenization) already materializes full results, the memory benefit is negligible compared to the cost of added code complexity and maintenance (e.g., handling slices, type narrowing, dual modes).
**Action:** Reverted the `LazyFormattedRerankInput` implementation in favor of the existing list comprehension as requested by the maintainer.
