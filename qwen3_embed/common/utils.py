...
def mean_pooling(input_array: NumpyArray, attention_mask: NDArray[np.int64]) -> NumpyArray:
    # Broadcasting with shape (Batch, Seq, 1) instead of np.tile is faster and uses less memory.
    # Explicitly casting the mask to the input array's dtype avoids type promotion overhead.
