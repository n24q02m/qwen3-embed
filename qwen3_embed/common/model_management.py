...
class ModelManagement(Generic[T]):
    ...
    def _try_download_hf(
        cls, hf_source: str, cache_dir: str, extra_patterns: list[str], kwargs: dict[str, Any]
    ) -> Path | None:
        try:
            return Path(
                cls.download_files_from_huggingface(
            ...
            logger.error(
                f"Could not download model from HuggingFace: {e} Falling back to other sources."
            )
            return None
        finally:
            enable_progress_bars()
    ...
    @classmethod
    def _try_download_gcs(cls, model: Any, cache_dir: str, local_files_only: bool) -> Path | None:
        try:
            return cls.retrieve_model_gcs(
                model.model,
                str(model.sources.url),
                str(cache_dir),
    ...
    def download_model(cls, model: T, cache_dir: str, retries: int = 3, **kwargs: Any) -> Path:
        ...
        if hf_source:
            cached_path = cls._check_hf_cache(
                hf_source, cache_dir, extra_patterns, model.model_file, kwargs
            )
            if cached_path:
                return cached_path
        ...
        while retries > 0:
            ...
            if hf_source and not local_files_only:
                path = cls._try_download_hf(hf_source, cache_dir, extra_patterns, kwargs)
                if path:
                    return path

            if url_source or local_files_only:
                path = cls._try_download_gcs(model, cache_dir, local_files_only)
                if path:
                    return path
