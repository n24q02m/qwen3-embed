...
# If adding tokens without "<|", _sanitize_input's fast-path must be updated.
...
class Qwen3CrossEncoderGGUF(TextCrossEncoderBase):
    ...
    def _sanitize_input(text: str) -> str:
        ...
        # ⚡ Bolt: C-level substring fast-path avoids regex search overhead for typical clean inputs
        if "<|" not in text:
            return text

        # Fast path to avoid regex substitution overhead on clean text (~50% faster for clean inputs)
