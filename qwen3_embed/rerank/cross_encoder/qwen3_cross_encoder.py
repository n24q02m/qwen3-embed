...
FORBIDDEN_TOKENS_PATTERN = re.compile("|".join(re.escape(t) for t in FORBIDDEN_TOKENS))
...
class Qwen3CrossEncoder(OnnxTextCrossEncoder):
    ...
    def _sanitize_input(text: str) -> str:
        ...
        # ⚡ Bolt: Fast C-level substring check avoids iteration overhead on clean inputs
        if "<|" not in text:
            return text

        # ⚡ Bolt: Fast path check with re.search before re.subn loop avoids overhead on clean text
        if not FORBIDDEN_TOKENS_PATTERN.search(text):
            return text

        # SECURITY: Prevent prompt injection bypass via iterative payload construction.
        while True:
            text, num_subs = FORBIDDEN_TOKENS_PATTERN.subn("", text)
            if num_subs == 0:
                break
        return text
