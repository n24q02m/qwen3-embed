...
class OnnxTextModel(OnnxModel[T]):
    ...
    def onnx_embed(
        ...
        **kwargs: Any,
        ...
        # Optimization: Use dtype=np.int64 directly to avoid unnecessary casting and copying
        input_ids = np.array([e.ids for e in encoded], dtype=np.int64)
        attention_mask = np.array([e.attention_mask for e in encoded], dtype=np.int64)
        input_names = {node.name for node in self.model.get_inputs()}  # type: ignore[union-attr]

        # Optimization: Reuse input_ids and attention_mask arrays without copying
        ...
        if "token_type_ids" in input_names:
            # Optimization: Use np.zeros_like for faster creation of token_type_ids
