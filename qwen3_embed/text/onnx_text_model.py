...
class OnnxTextModel(OnnxModel[T]):
    ...
    def _load_onnx_model(self, config: OnnxModelConfig) -> None:
        super()._load_onnx_model(config=config)
        self.tokenizer, self.special_token_to_id = load_tokenizer(model_dir=config.model_dir)
